...
public:
	...
	*/
	...
	void BindParam(int32 idx, T(&value)[N]) {
		//_dbConnection.BindParam(idx + 1, (const BYTE*)value, static_cast<int32>(sizeof(T)) * N, &_paramIndex[idx]);
	...
	*/
	...
	void BindCol(int32 idx, T(&value)[N]) {
		//_dbConnection.BindCol(idx + 1, value, static_cast<int32>(sizeof(T)) * N, &_columnIndex[idx]);
...
protected:
	...
	SQLLEN _columnIndex[ParamCount > 0 ? ParamCount : 1]; // 이 부분 확인할 필요가 있다.(그대로 치긴함)
